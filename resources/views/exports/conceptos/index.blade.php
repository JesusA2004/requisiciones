...
<head>
  <meta charset="utf-8">
  <title>{{ $meta['title'] ?? 'Reporte de Conceptos' }}</title>
  <style>
...
<body>
  ...
  <div class="header">
    <div>
      <p class="title">{{ $meta['title'] ?? 'Reporte de Conceptos' }}</p>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th style="width:18%">Actualizado</th>
      </tr>
    </thead>
    <tbody>
      @forelse(($rows ?? []) as $r)
      ...
      @empty
        <tr>
          <td colspan="5" class="muted">Sin resultados con los filtros actuales.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
